...
class EdgeDetectionBot:
    ...
    def run_loop(self):
        """Integrated loop: High-frequency Take Profit checks + Regular Market Scanning"""
        ...
        while self.running:
            ...
            # 1. Hourly Ticker Lock Reset
            if int(now) % 3600 < 10:
                self.edge_detector.reset_locks()

            try:
                # --- FIX: HIGH FREQUENCY TAKE PROFIT CHECK ---
                # Run this every 5 seconds to catch rapid price spikes
                if not self.paused:
                    self.position_manager.manage_take_profit()

                # 2. UPDATE SPOT PRICES & MOMENTUM
                if now - last_spot_update >= spot_update_interval:
                    self._update_spot_prices()
                    last_spot_update = now

                # 3. MAIN SCAN INTERVAL (Slower loop for finding new trades)
                ...
                # Sleep briefly to control loop speed and CPU usage
                time.sleep(5)
