...
class Market15mScanner:
    def __init__(self, client: KalshiClient, config: Dict):
        ...
        logger.info("✅ 15-minute market scanner initialized (event-based)")
    ...
    def _evaluate_market(self, market: Dict) -> Optional[Dict]:
        ...
        if not ticker or ('KXBTC' not in ticker and 'KXETH' not in ticker): return None
        symbol = 'BTC' if 'KXBTC' in ticker else 'ETH'
        ...
        if not (self.strategy_config.get('min_minutes_to_close', 2) <= minutes_to_close <= 60):
            return None
        ...
        # Capture best prices AND sizes
        return {
            'ticker': ticker, 'symbol': symbol, 'title': market.get('title', ''),
            ...
            'yes_ask_size': yes_orders[0][1], 'no_ask_size': no_orders[0][1], # SIZE DATA
    ...
    def get_market_summary(self, opportunities: List[Dict]) -> Dict:
        if not opportunities: return {'count': 0, 'symbols': {}}
        symbols = {}
        for opp in opportunities:
            s = opp.get('symbol', 'Unknown')
            symbols[s] = symbols.get(s, 0) + 1
        return {'count': len(opportunities), 'symbols': symbols}
